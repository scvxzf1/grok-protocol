...
class SingleInstanceLock:
    """Hold the P0 cross-platform lock for the supervisor lifetime."""

    def __init__(self, path: Path):
        self.path = Path(path)
        self._lock = CrossProcessFileLock(self.path, timeout=0.0)
...
class ServerMailSupervisor:
    def __init__(
        ...
        *,
        ...
        self.lifecycle_status = "created"
        self.started_at_epoch = 0
        self.last_epoch_started_at_epoch = 0
    ...
    def _load_master_and_latest(self) -> None:
        try:
            with CrossProcessFileLock(
                ...
                # Preserve deterministic epoch rotation while accepting records
                # appended to the durable master between reconciliations.  Drop
                # removed keys and append new keys in their on-disk order.
                self.order = [key for key in self.order if key in master] + [
                    key for key in master if key not in self.order
                ]
    ...
    def _write_state(
        ...
        *,
        ...
        allowed_status = {"created", "running", "complete", "stopped", "limit", "error"}
        requested_status = str(status or self.lifecycle_status or "running").strip().lower()
        self.lifecycle_status = requested_status if requested_status in allowed_status else "error"
        # Aggregate counters and lifecycle metadata only; no identity, path,
        # SSO, proxy, mailbox, token, or credential material is persisted here.
        payload = {
            "schema_version": 1,
            "status": self.lifecycle_status,
            "updated_at_epoch": int(time.time()),
            "started_at_epoch": int(self.started_at_epoch),
            "last_epoch_started_at_epoch": int(self.last_epoch_started_at_epoch),
    ...
    def _patch_runtime_config(self, count: int) -> None:
        ...
        config.update(
            {
                ...
                "local_turnstile_max_inflight": self.turnstile_workers,
        ...
        # Current BatchService owns static HTTP proxy selection through the P3
        # exclusive lease API.  Use its canonical egress normalizer so both the
        # canonical and tui_* compatibility keys stay coherent.
        apply_egress_mode_to_config(config, "http")
    ...
    def _run_epoch(self, selected: Sequence[str]) -> Dict[str, int]:
        self.last_epoch_started_at_epoch = int(time.time())
    ...
    def run(self, *, max_epochs: int = 0) -> int:
        ...
        while not self.stop_requested:
            ...
            if self.stop_requested:
                self._write_state(after_convert, status="stopped")
                return 130
            if after_convert.complete >= after_convert.planned:
                self._write_state(after_convert, status="complete")
            ...
            if after_convert.pending:
                ...
                if max_epochs > 0 and self.epoch > max_epochs:
                    self._write_state(after_convert, status="limit")
                    return 2
                ...
                try:
                    ...
                    try:
                        failed_state = self.reconcile()
                        self._write_state(failed_state, status="error")
                    except Exception:
                        pass
                    raise
            ...
            self._write_state(
                ...
                status=(
                    ...
                    else ("stopped" if self.stop_requested else "running")
                ),
            )
        ...
        try:
            final_state = self.reconcile()
            self._write_state(final_state, status="stopped")
        except Exception:
            pass
        return 130
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--lock", default="", help=argparse.SUPPRESS)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Ownership is derived from the state set itself.  A caller-selected lock
    # path could let two supervisors mutate the same master/work/state files.
