...
- 自定义配置：`python webui_app.py --config /path/to/config.json`，也可设置 `XAI_CONFIG_PATH`
...
PIVOT: if proxy 407 → refresh proxy credentials or use a fresh pool; if graph invalid_grant → yyds/cf
```
...
外部 Outlook/Graph master 较大、需要固定小批次断点续跑时：

```bash
python server_mail_supervisor.py \
  --config /private/runtime-config.json \
  --master /private/mail-master.txt \
  --work /private/mail-work.txt \
  --proxy /private/proxies.txt \
  --output /private/xai-credentials \
  --state /private/mail-supervisor.state.json
```

- master 行格式仍是 `email----password----client_id----refresh_token`；work 每轮原子重建，
  相邻的 `work.txt.used` 与 `work.txt.lock` 由 Graph worker 维护。
...
- master/work/proxy/state/凭证目录均应位于仓库外或受 `.gitignore` 保护。
...
| `server_mail_supervisor.py` | Outlook/Graph master 的断点续跑与小批次监督 |
