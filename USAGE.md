...
| 字段 | 说明 |
| --- | --- |
| `proxy_mode` | `auto` / `none` / `direct` / `pool` |
| `proxy` | `direct` 模式的单个代理；支持 `host:port:user:pass` 或标准 URL |
| `proxy_file` | `pool` 模式的代理列表文件 |
| `proxy_random` | 从代理池随机选择条目 |
| `proxy_slot_sticky` | 活动任务独占代理池槽位，任务结束后回收 |
| `embedded_proxy_enabled` | 是否由项目内置 mihomo 节点接管任务出口 |
| `proxy_subscription_url` | mihomo 节点订阅地址 |
...
`proxies.txt` 每行一个代理。启用 `proxy_slot_sticky` 后，活动任务会获得互不重复的固定条目；可用条目少于并发数时，批处理器会收敛并发。

开启 `embedded_proxy_enabled` 后，链路为：`本机程序 -> 项目内置 mihomo 节点 -> 目标站点`。内置节点会接管任务出口，实际并发由本机 worker 与 Turnstile 浏览器槽位决定。

使用本机配置启动 WebUI：

```powershell
python webui_app.py --config config.local.json --host 127.0.0.1 --port 33843
```
...
| `turnstile_headless` | `true` 时使用 Chrome 原生 `headless=new`，不启动 Xvfb 或可见窗口 |
| `local_turnstile_max_workers` | 本地浏览器 Turnstile 并发上限（默认 5，范围 1~6666；仅 `turnstile_provider=local` 生效） |
| `submit_workers` | 注册提交阶段并发（默认 5） |
| `browser_idle_ttl_sec` | 空闲浏览器回收时间，默认 90 秒；设为 0 可保留到其它回收条件触发 |
| `browser_max_rss_mb` | 单个 Chrome 进程树 RSS 回收阈值，默认 1024 MiB；设为 0 关闭该阈值 |
...
| 字段 | 值 | 说明 |
| --- | --- | --- |
| `credential_flow` | `pkce` | 注册后在同一会话直接换取凭证，默认推荐 |
| `credential_flow` | `sso_device` | 先保存 SSO，再用 Device Flow 二次转换 |
| `sso_convert_retries` | 整数 | 仅 `sso_device` 使用 |
| `sso_convert_cooldown` | 秒 | 仅 `sso_device` 使用 |

旧的 `tui_run_mode`、`tui_proxy_mode`、`tui_sso_*`、`run_target_mode` 只作为读取别名；保存后统一迁移到简洁字段。
...
```bash
python grok_register_ttk.py http register \
  --proxy http://user:password@proxy.example:10000 \
  --mail-config config.json \
...
| `turnstile-capture` | 真浏览器捕获 Turnstile；配合 `--headless` 使用原生 `headless=new` |

公共代理参数：`--proxy` / `--proxy-file` / `--proxy-random` / `--proxy-index` / `--timeout`。WebUI/TUI 支持 `auto`、`none`、`direct`、`pool`；内置 mihomo 出口由批处理器自动注入。
...
| 代理 CONNECT 407 | 代理账密错误或失效 | 更新代理 URL 或代理池条目 |
| mihomo 链路超时 | 当前内置节点异常 | 在 WebUI 重新预检/切换内置节点 |
...
PIVOT: if proxy 407 → refresh proxy credentials; if graph invalid_grant → yyds/cf
```
