...
class BrowserPathDetectTests(unittest.TestCase):
    ...
    def test_detect_system_chrome_path_prefers_program_files_order(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            program_files = base / "Program Files"
            program_files_x86 = base / "Program Files x86"
            local = base / "LocalAppData"
            expected = self._make_windows_chrome(program_files)
            self._make_windows_chrome(program_files_x86)
            self._make_windows_chrome(local)
            with mock.patch.dict(
                os.environ,
                {
                ...
                detected = detect_system_chrome_path()
        self.assertEqual(detected, str(expected.resolve(strict=False)))
    ...
    def test_resolved_browser_path_auto_detects_when_unset(self):
        ...
        # Build an absolute path with the host platform's path semantics.  A
        # POSIX-looking path is relative on Windows (``Path('/usr/bin')``),
        # which made this unit test fail before it reached the mocked file
        # checks.
