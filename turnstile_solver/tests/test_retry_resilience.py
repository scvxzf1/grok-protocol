...
class PersistentBrowserRetryTests(unittest.TestCase):
    ...
    def test_transient_challenge_recycles_and_recovers_inside_deadline(self):
        ...
        self.assertEqual(result.extras["retry_history"][0]["error_code"], "600010")
    ...
    def test_retry_exhaustion_has_stable_web_diagnostics(self):
        ...
        self.assertEqual(pool.stats.failure_categories["turnstile_challenge_transient"], 1)
        self.assertEqual(
    ...
    def test_retry_does_not_start_after_overall_deadline_is_nearly_exhausted(self):
        pool = self._pool(attempts=3)
        slot = _FakeSlot("slot-1", _challenge_failure("600010"))
        slot.solve = Mock(wraps=slot.solve)
        pool._acquire = Mock(return_value=slot)
        ticks = iter([0.0, 0.0, 9.5, 9.5, 9.5])

        with MockMonotonic(ticks):
            result = pool.solve(SolveRequest(timeout_sec=10))

        self.assertFalse(result.ok)
        self.assertEqual(pool._acquire.call_count, 1)
        slot.solve.assert_called_once()
        self.assertEqual(pool.stats.retry_attempts, 0)

    def test_capture_does_not_round_subsecond_budget_past_deadline(self):
        pool = self._pool(attempts=1)
        slot = _FakeSlot("slot-1", _challenge_failure("600010"))
        slot.solve = Mock(wraps=slot.solve)
        pool._acquire = Mock(return_value=slot)

        with MockMonotonic(iter([0.0, 9.5, 9.5])):
            result = pool.solve(SolveRequest(timeout_sec=10))

        self.assertFalse(result.ok)
        self.assertIn("insufficient capture budget", result.error)
        slot.solve.assert_not_called()
...
class MockMonotonic:
    ...
    def __enter__(self):
        self.patcher = patch("src.browser_runtime.time.monotonic", side_effect=self.values)
        return self.patcher.__enter__()

    def __exit__(self, exc_type, exc, traceback):
        return self.patcher.__exit__(exc_type, exc, traceback)
